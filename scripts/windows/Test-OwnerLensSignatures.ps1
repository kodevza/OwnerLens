<#
.SYNOPSIS
Reports and optionally enforces Authenticode signature status for OwnerLens artifacts.
#>
...
if (-not $IsWindows) {
  throw "Test-OwnerLensSignatures.ps1 is Windows-only because Get-AuthenticodeSignature is Windows-only."
}
...
if (-not $results) {
  throw "No OwnerLens files with Authenticode signature support were found."
}
...
if ($RequireValid) {
  ...
  if ($invalid) {
    throw "One or more OwnerLens signatures are not valid: $($invalid.Path -join ', ')"
  }
}
...
if ($RequireTimestamp) {
  ...
  if ($missingTimestamp) {
    throw "One or more OwnerLens signatures are missing a timestamp: $($missingTimestamp.Path -join ', ')"
  }
}
