create table if not exists external_ownership_evidence_items (
...
select
  ...
  case lower(trim(owner_type))
    when 'ownercustomlog' then 'ownerCustomLog'
    else 'ownerCustom'
  end as "ownerType",
...
union all
select * from runtime_external_principal_owner_evidence
union all
