import { applyRuntimeCollectionFilters, buildPaginatedCollection } from "./collections";
...
test("keeps the public paginated collection shape", () => {
  const collection = buildPaginatedCollection("test", [{ id: "first", displayName: "Alpha" }], {});

  expect(Object.keys(collection).sort()).toEqual(["collectionId", "columns", "count", "page", "pageSize", "rows"]);
  expect(collection).toMatchObject({
    collectionId: "test",
    rows: [{ id: "first", displayName: "Alpha" }],
