...
export function SelectableGenericTable<TRow>(props: SelectableGenericTableProps<TRow>) {
  ...
  const [selectionSortRules, setSelectionSortRules] = useState<SortRule[]>(() =>
    isRemoteTableProps(tableProps) ? [] : (tableProps.sortRules ?? [])
  );
  ...
  const selectionOverlay =
    renderSelectionOverlay && resolvedSelectedRowKeys.length > 0
      ? renderSelectionOverlay({
          ...
          selectedRowKeys: resolvedSelectedRowKeys,
          sortRules: selectionSortRules
        })
      : null;
        ...
        return (
          <SelectionCheckbox
            ...
            onChange={(selected) => {
              ...
              setSelectAllMatchingFilters(selected);
    ...
    return (
      <div className="mb-[150px]">
        <GenericRemoteTable
          ...
          onRuntimeControlsChange={handleRuntimeControlsChange}
        />
        {selectionOverlay}
      </div>
    );
  ...
  const handleSortRulesChange = (nextSortRules: SortRule[]) => {
    setSelectionSortRules(nextSortRules);
    tableProps.onSortRulesChange?.(nextSortRules);
  };
