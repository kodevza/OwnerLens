...
test("projects every active resource group owner candidate into the collection row", async () => {
  const rows = await withDuckDb(async ({ connection }) => {
    await insertAzureResourceGroupRows(connection, [
      resourceGroup("rg-multiple-owners", {
        owner: "fallback@example.test",
        costCenter: "cc-1001",
        ownerGroup: "Platform-Team"
      })
    ...
    return queryAzureResourceGroupOwnershipCollectionRows(connection);
  });
  ...
  expect(rows).toEqual([
    expect.objectContaining({
      ...
      confidence: "high",
      ownerCandidates: [
        expect.objectContaining({ displayName: "platform-team", rank: 1 }),
        expect.objectContaining({ displayName: "cc-1001", rank: 2 }),
        expect.objectContaining({ displayName: "fallback@example.test", rank: 3 })
      ]
    })
  ]);
});
