...
test("defines local report runtime REST endpoints", async () => {
  ...
  const runtime = {
    ...
    generatePowerShellScript: jest.fn(
      (request: {
        collectionId?: "azureResources.resourceGroupOwnership" | "entra.servicePrincipals" | "entra.managedIdentities";
        selection: { selectedRowKeys?: string[] };
        templateId: "setResourceGroupOwnerTag" | "setResourceGroupOwnerGroupTag" | "setServicePrincipalOwnerTag";
      }) =>
        Promise.resolve({
          ...
          body: "# Generated by OwnerLens\nSet-AzResourceGroup",
          count: request.selection.selectedRowKeys?.length ?? 1,
          targetIds: request.selection.selectedRowKeys ?? ["sub-1:rg-1"]
        })
    ),
  ...
  await expect(
    powershellScriptEndpoint.handle({
      req: {},
      url: new URL(
        "http://localhost/api/data/scripts/powershell?template=setResourceGroupOwnerGroupTag&filter[0][column]=confidence&filter[0][value]=high&selectedRowKey=sub-1%3Arg-1&sort[0][column]=resourceGroup&sort[0][direction]=asc"
      )
    })
  ).resolves.toMatchObject({
    kind: "powershellScript",
    ...
    fileName: "ownerlens-set-resource-group-owner.ps1",
    body: expect.stringContaining("Set-AzResourceGroup")
  });
  await expect(
  ...
  expect(runtime.generatePowerShellScript).toHaveBeenCalledWith({
